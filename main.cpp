     ...
     try {
          for (;;) {
               try {
                    ...
                    // Split the line into words and lookup the 
                    // appropriate function.Complain or call it.
                    wordvec words = split(line, " \t");
                    //DEBUGF('y', "words = " << words);
                    if (words.front().compare("#") != 0) {
                         command_fn fn = find_command_fn(words.at(0));
                         fn(state, words);
                    }
               }
               catch (command_error& error) {
                    // If there is a problem discovered in any 
                    // function, an exn is thrown and printed 
                    // here.                
